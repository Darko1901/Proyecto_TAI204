...
API_BASE_URL = os.environ.get("API_BASE_URL", "http://macuin_api:8080")
...
def patch_multipart(endpoint, data, files, token):
    try:
        response = requests.patch(
            f"{API_BASE_URL}{endpoint}",
            data=data,
...
def delete_data(endpoint, token):
    try:
